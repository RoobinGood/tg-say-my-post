...
- **FR-004**: Конфигурация (model_id, speaker, язык, частота дискретизации, формат, путь вывода, устройство/режим вычислений, уровень логирования) должна задаваться через отдельный конфиг, переопределяемый переменными окружения.
...
- Применяется офлайн режим: модель Silero и веса доступны локально или загружаются однократно при первом запуске.
