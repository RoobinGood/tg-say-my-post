...
def test_add_period_per_paragraph(text, expected):
    assert asyncio.run(preprocess_text(text)) == expected


def test_replace_leading_emoji_and_remove_all():
    assert asyncio.run(preprocess_text("😀Привет 😀мир")) == "-Привет мир."


def test_remove_only_emojis_results_empty():
    assert asyncio.run(preprocess_text("😀😁")) == "-."


def test_square_bullet_is_removed_and_dash_added():
    assert asyncio.run(preprocess_text("▪️Пункт списка")) == "-Пункт списка."


def test_arrow_and_playing_card_removed():
    result = asyncio.run(preprocess_text("↘hello 🂡world"))
...
def test_capitalize_paragraphs():
    assert asyncio.run(preprocess_text("привет мир")) == "Привет мир."
    assert asyncio.run(preprocess_text("первый абзац\n\nвторой абзац")) == "Первый абзац.\n\nВторой абзац."


def test_remove_urls():
    assert asyncio.run(preprocess_text("смотри https://example.com тут")) == "Смотри тут."
    assert asyncio.run(preprocess_text("ссылка: http://test.ru")) == "Ссылка:."
